...
func TestHandlerValidationWritesFailure(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"financial report type", h.GetIncomeStatement, "/?report_type=monthly", "", 400, "report_type must be quarterly or annual"},
		{"financial year range", h.GetBalanceSheet, "/?from_year=2026&to_year=2025", "", 400, "from_year must not be after to_year"},
		{"financial annual quarter", h.GetCashFlow, "/?report_type=annual&quarter=1", "", 400, "quarter is only valid for quarterly reports"},
		{"financial page", h.GetIncomeStatement, "/?page=0", "", 400, "page must be a positive integer"},
		{"financial page size", h.GetIncomeStatement, "/?per_page=101", "", 400, "per_page must not exceed 100"},
