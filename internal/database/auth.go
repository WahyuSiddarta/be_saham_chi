...
ALTER TABLE t_user ADD COLUMN IF NOT EXISTS role_id INTEGER REFERENCES t_role(role_id);
		...
		VALUES
			...
			('portfolio.stock.read', 'Read stock holdings and transactions'),
			('portfolio.stock.create', 'Create stock transactions'),
			('portfolio.stock.update', 'Update stock transactions'),
			('portfolio.stock.delete', 'Delete stock transactions'),
		...
		WHERE ar.code IN (
			...
			'portfolio.stock.read',
			'portfolio.stock.create',
			'portfolio.stock.update',
			'portfolio.stock.delete',
			'portfolio.bond.read',
